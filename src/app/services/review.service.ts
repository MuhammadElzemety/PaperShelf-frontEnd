...
export class ReviewService {
  ...
  searchApprovedReviews(searchText: string, page: number = 1, limit: number = 10): Observable<{ success: boolean, data: Review[], pagination: Pagination }> {
    const params = new HttpParams()
      .set('search', searchText)
      .set('page', page.toString())
      .set('limit', limit.toString());

    return this.http.get<{ success: boolean, data: Review[], pagination: Pagination }>(`${this.baseUrl}/approved/search`, { params });
  }
}
